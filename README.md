...
- **✏️ 简单配置**: 只需编写简单的XML文件
- **🎯 精确匹配**: 准确替换指定的文本内容
- **⚙️ 设置界面**: 图形化设置界面，控制日志级别和性能选项
- **📊 性能监控**: 实时监控框架性能和翻译效果
- **🛡️ 安全模式**: 智能检测复杂方法，自动切换安全模式
...
框架提供了完整的图形化设置界面，可以在游戏中轻松调整各种选项：

### 🎛️ 访问方式
1. 游戏内进入 **选项** → **Mod设置**
2. 找到 **Universal Translation Framework**
3. 点击进入设置页面
...
- **日志级别**: 控制显示哪些日志信息
  - `Error`: 仅错误信息（推荐生产环境）
  - `Warning`: 错误 + 警告
  - `Message`: 错误 + 警告 + 一般消息（推荐日常使用）
  - `Debug`: 所有日志（推荐调试时使用）

#### 性能设置
- **性能监控**: 监控转译器性能
- **强制安全模式**: 使用更安全但较慢的转译模式
- **并行线程数**: 控制并行处理的线程数（0=自动）

#### 调试选项
- **状态机详细日志**: 显示状态机检测的详细信息
- **补丁统计**: 显示翻译补丁应用统计
...
### 💡 使用建议
- **日常使用**: Message级别 + 关闭调试选项
- **开发调试**: Debug级别 + 启用所有监控
- **性能优化**: Error级别 + 关闭所有额外功能

详细设置说明请参考 [`SETTINGS_GUIDE.md`](markdown/SETTINGS_GUIDE.md)
